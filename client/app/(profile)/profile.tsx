...
import { ArrowLeft, Settings, Trophy, Activity, Users, Calendar, Clock, Star, Award } from 'lucide-react-native';
        ...
        <SafeAreaView className={`flex-1 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`} edges={['top']}>
            ...
            >
                ...
                <MotiView
                    ...
                    className="px-4 mt-4"
                >
                    <View className={`rounded-2xl p-1 flex-row ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`}>
                        {tabs.map((tab) => (
                            <TouchableOpacity
                                key={tab.id}
                                onPress={() => handleTabChange(tab.id)}
                                className={`flex-1 py-2.5 rounded-xl flex-row items-center justify-center space-x-2 ${
                                    activeTab === tab.id
                                        ? isDark
                                            ? 'bg-gray-700'
                                            : 'bg-white shadow-sm'
                    ...
                    <AnimatePresence>
                        <MotiText
                            ...
                            className={`text-xs mt-2 text-center ${isDark ? 'text-gray-400' : 'text-gray-500'}`}
                        >
