...
const register = async (req, res) => {
    ...
    if (!user_name?.trim() || !user_email?.trim() || !gender || !password) {
        return res.status(400).json({
            success: false, error: "Please fill all the required fields"
        });
    }
    ...
    try {
        ...
        return res.status(500).json({
            success: false, error: "Unable to complete registration"
        });
    }
};
    ...
    try {
        ...
        await prisma.user.update({
            where: {id: user.id}, data: {
                resetPasswordToken: crypto.createHash('sha256').update(resetToken).digest('hex'), resetPasswordExpiry
            }
        });
        ...
        if (user?.user_id) {
            await prisma.user.update({
                where: {user_id: user.user_id}, data: {
